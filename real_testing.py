...
excel_filename = os.getcwd() + f"{weights}/{network}/{chosen_loss}/{learning_param}/{image_type}" + f'/real_results.xlsx'
...
for model_weights in glob.glob(os.getcwd() + f'{weights}/{network}/{chosen_loss}/{learning_param}/{image_type}/*.pth'):
    ...
    worksheet.write(row, 3, 'output label')
    worksheet.write(row, 4, 'mod output label')
    row += 1
