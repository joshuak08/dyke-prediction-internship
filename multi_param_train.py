...
from loss_functions import L1AngularLoss, L2AngularLoss, MultiLoss
import torch
...
parser.add_argument('--strikeWeight', type=float, default=1.0)
parser.add_argument('--openingWeight', type=float, default=1.0)
...
numParam = len(learningParam)
strLearningParam = "_".join(learningParam)
...
strike_weight = args.strikeWeight
opening_weight = args.openingWeight
...
model_choices = {"swin": SwinTransformerV2(img_size=image_size, num_classes=numParam), 'convnext': ConvNeXt(), 
                 'deit': vit_models(img_size=image_size, num_classes=numParam), 'crossvit': VisionTransformer(img_size=(image_size, image_size), num_classes=numParam),
...
loss_function = MultiLoss(strike_weight, opening_weight)
...
for epoch in iterator:
    ...
    for i, batch in enumerate(valloader):
        ...
        loss = loss_function(val_output, batch)
    ...
    if epoch % save_model_epoch == 0:
        if not os.path.exists(os.getcwd() + f"{outputDir}/{network}/{strLearningParam}/{image_type}"):
            os.makedirs(os.getcwd() + f"{outputDir}/{network}/{strLearningParam}/{image_type}", exist_ok=True, mode=0o777)

        torch.save(model.state_dict(), os.path.join(os.getcwd() + f"{outputDir}/{network}/{strLearningParam}/{image_type}", f"ep_{epoch}.pth"))
    ...
    if epoch_val_loss < best_val_loss:
        ...
        torch.save(best_model, os.path.join(os.getcwd() + f"{outputDir}/{network}/{strLearningParam}/{image_type}", "best_model.pth"))
